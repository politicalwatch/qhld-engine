...
class BulletinsExtractor(InitiativeExtractor):
    ...
    def should_extract_content(self):
        ...
        has_imported_bulletins = (
            has_extra and "imported_bulletins" in self.initiative["extra"]
        )
        has_different_bulletins = has_imported_bulletins and self.initiative["extra"][
            "imported_bulletins"
        ] != len(urls)
    ...
    def retrieve_bulletin(self):
        ...
        try:
            for url in urls:
                bulletin_tree = document_fromstring(
                    self.api.get_url(f"{self.BASE_URL}{url}").text
                )
                ...
                for link in more_links:
                    ...
                    page_bulletin_tree = document_fromstring(
                        self.api.get_url(f"{page_url}").text
                    )
...
class NonExclusiveBulletinExtractor(InitiativeExtractor):
    ...
    def extract_bulletin_metadata(self):
        text = self.node_tree.xpath("//ul[@class='boletines']/li[1]/div[1]")[
            0
        ].text_content()
        link = self.node_tree.xpath("//ul[@class='boletines']/li[1]/div[2]/a[1]/@href")[
            0
        ]
    ...
    def extract_bulletin_content(self):
        ...
        try:
            ...
            for link in more_links:
                ...
                element = page_bulletin_tree.xpath(
                    "//div[contains(@class, 'textoIntegro')]"
                )[0]
                cleanup_content += self.cleanup_content(element)
        except Exception:
    ...
    def cleanup_content(self, element):
        ...
        full_content = (
            full_content.replace("<br><br><br><br>", "\n")
            .replace("<br><br><br>", "\n")
            .replace("<br>", " ")
        )
        full_content = (
            full_content.replace("    ", " ").replace("   ", " ").replace("  ", " ")
        )
