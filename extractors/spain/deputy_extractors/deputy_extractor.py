...
class DeputyExtractor:
    ...
    def extract(self):
        ...
        (
            self.deputy["gender"],
            self.deputy["constituency"],
        ) = self.get_gender_and_constituency_from(self.get_text_by_css(".cargo-dip"))
    ...
    def extract_dates(self):
        ...
        end_date = self.clean_str(date_elements[1].text_content()).replace(
            "Causó baja el ", ""
        )[:10]

        self.deputy["start_date"] = self.parse_date(
            self.clean_str(date_elements[0].text_content()).replace(
                "Condición plena: ", ""
            )[:10]
        )
        if end_date != "":
    ...
    def extract_social_media(self):
        ...
        for link in social_links:
            img_src = link.getchildren()[0].get("src") if link.getchildren() else ""
    ...
    def extract_extras(self):
        ...
        for link in links:
            self.deputy["extra"]["declarations"][
                self.clean_str(link.text)
            ] = self.BASE_URL + link.get("href")

    def extract_from_text(self):
        birthday_paragraph = self.clean_str(
            self.get_by_xpath(
                "//h3[normalize-space(text()) = 'Ficha personal']/following-sibling::p[1]"
            )[0].text
        )
        birthday = birthday_paragraph.replace("Nacido el ", "").replace(
            "Nacida el ", ""
        )[:10]
        if birthday != "":
        ...
        legislatures_paragraph = self.clean_str(
            self.get_by_xpath(
                "//h3[normalize-space(text()) = 'Ficha personal']/following-sibling::p[2]"
            )[0].text
        )
        self.deputy["legislatures"] = (
            legislatures_paragraph.replace("Diputada", "")
            .replace("Diputado", "")
            .replace(" de la ", "")
            .replace(" Legislaturas", "")
            .replace("y ", "")
            .split(", ")
        )
        ...
        bio = self.clean_str(
            self.get_by_xpath(
                "//h3[normalize-space(text()) = 'Ficha personal']/parent::div"
            )[0].text_content()
        )
        bio = (
            bio.replace("Ficha personal", "")
            .replace(birthday_paragraph, "")
            .replace(legislatures_paragraph, "")
        )
