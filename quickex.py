...
def print_help():
    ...
    print(
        "Apply task: send-alerts, generate-alert, topic-alignment, tagger, untagger, stats, footprint, extractor"
    )
...
def long_questions(args):
    query = {"content.100000": {"$exists": True}}
    ...
    for initiative in initiatives:
        ...
        if content_len > 531800 and content_len < 532100:
            if (
                not tagged_initiative
                and all(tagged.has_topics for tagged in initiative.tagged)
                and len(initiative.tagged) == 2
            ):
    ...
    for initiative in selected_initiatives:
        print(
            f"Copying tags to initiative {initiative['reference']} {initiative['id']}. {counter} out of {total}"
        )
